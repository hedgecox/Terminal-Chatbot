...
const Computer = (text, setLines, setChatbotActive) => {
  const compute = () => {
    ...
    const words =
      lowerText.indexOf(" ") > 0 ? lowerText.split(" ") : [lowerText];
    switch (words[0]) {
      case "help":
        switch (words[1]) {
          case "color":
...
chat			Activate chatbot`;
      ...
      default:
        return `'${text}' is not recognized as an internal or external command, operable program or batch file. Type 'Help' or 'Chat' to get started`;
    }
  };
